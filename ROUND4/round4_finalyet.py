...
class Trader:
    ...
    '''
    #list of symbols
    products = ['PEARLS', 'BANANAS', 'COCONUTS', 'PINA_COLADAS','DIVING_GEAR','BERRIES','BAGUETTE','DIP','UKULELE','PICNIC_BASKET']
    symbols = ['PEARLS', 'BANANAS','COCONUTS', 'PINA_COLADAS','DIVING_GEAR','BERRIES','BAGUETTE','DIP','UKULELE','PICNIC_BASKET']
        ...
        for symbol in self.symbols:
            ...
            if self.strategy[symbol] == 'diving':
                ...
                if state.timestamp >= 100:
                    if abs(dolphins - self.dolphins_last_round > 7):
                    ...
                    if state.timestamp - self.last_dolphin_jump < 1000: #right after jump is detected buy or sell (for 10 rounds)
                        if self.buy_gear:
                            if len(order_depth.sell_orders) > 0:
                                ...
                                orders.append(Order(symbol, best_ask-2, volume/2))
                            ...
                            if len(order_depth.buy_orders) > 0:
                                ...
                                orders.append(Order(symbol, best_bid-2, volume/2))
                    ...
                    if (state.timestamp - self.last_dolphin_jump) > time_until_exit & current_position != 0: #after time_until_exit is over, try to exit position
                        if self.buy_gear:
                            if len(order_depth.buy_orders) > 0:
                                best_bid = max(order_depth.buy_orders.keys())
                                volume = -self.position_limits[symbol] - current_position
                                orders.append(Order(symbol, best_bid+2, volume))
                                print("," + "sell," + str(volume) + ", ", best_bid)
                        else:
                            if len(order_depth.sell_orders) > 0:
                                best_ask = min(order_depth.sell_orders.keys())
                                volume = self.position_limits[symbol] - current_position
                                orders.append(Order(symbol, best_ask+2, volume))
                                print("," + "buy," + str(volume) + ", ", best_ask)
    ...
    def make_trades(self, acceptable_price: int,symbol: str, state: TradingState) -> List[Order]:
        ...
        for ask_price in market_ask_prices:
            ...
            if ask_price < acceptable_price+day:
                ...
                if symbol in ['PEARLS','PICNIC_BASKET','UKULELE','BAGUETTE']:
        ...
        if symbol=='DIVING_GEAR':
            ...
            d = 0.45 #fraction of position that we try to unload. Should be between 0 and 1
